...
class PersonaController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Persona'],$_POST['Usuario']))
		{
			...
			$modelusuario->RUT_PERSONA=$_POST['Persona']['RUT_PERSONA'];//para que valide
        	$valid=$model->validate();
        	$valid=$modelusuario->validate() && $valid;
			if($valid)
			{
				if($model->save())
				{
					$modelusuario->RUT_PERSONA=$model->RUT_PERSONA;
					$modelusuario->PASS_USUARIO=md5($modelusuario->PASS_USUARIO); //agregar esta linea antes del save() lo mismo en la funcion de modificar actionUpdate()
